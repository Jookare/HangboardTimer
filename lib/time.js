...
// Planned seconds for a partial workout of `hangs` completed hangs — hang time
// for each, plus the rest that follows all but the last (set rest on set
// boundaries, otherwise rep rest).
export const hangsPlannedSeconds = ({ reps, hangTime, repRest, setRest }, hangs) => {
  const n = Math.max(0, Math.floor(hangs));
  if (n === 0) return 0;
  let total = n * hangTime;
  for (let i = 1; i < n; i += 1) {
    total += i % reps === 0 ? setRest : repRest;
  }
  return total;
};
